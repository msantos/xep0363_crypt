...
%% API exports
-export([download/1, uri/1, fetch/1, decrypt/2, decrypt_file/2, encrypt/1,
         encrypt/2, encrypt_file/1, encrypt_file/2, iveckey/1]).
...
uri(URI) when is_list(URI) -> uri(list_to_binary(URI));
uri(<<"aesgcm://", URI/binary>>) ->
    case binary:split(URI, <<"#">>) of
      [Path, Key] -> {ok, list_to_binary(["https://", Path]), {aesgcm, Key}};
      _ -> {error, invalid}
    end;
uri(<<"https://", _/binary>> = URI) -> {ok, URI, plaintext};
uri(_) -> {error, unsupported}.
...
-spec decrypt_file(file:name_all(), binary() | key()) -> {ok, binary()} |
                                                         {error,
                                                          file:posix() | badarg | terminated |
                                                          system_limit}.
...
decrypt(PlainText, plaintext) -> PlainText;
...
-spec encrypt_file(file:filename_all()) -> {ok, {iodata(), binary()}} |
                                           {error,
                                            file:posix() | badarg | terminated | system_limit}.

encrypt_file(File) -> encrypt_file(File, {aesgcm, 16}).

-spec encrypt_file(file:filename_all(), {aesgcm, 12 | 16} | plaintext) -> {ok,
                                                                           {iodata(), binary()}} |
                                                                          {error,
                                                                           file:posix() | badarg |
                                                                           terminated |
...
-spec encrypt(binary(), {aesgcm, 12 | 16} | plaintext) -> {iodata(),
                                                           binary() | plaintext}.

encrypt(PlainText, plaintext) -> {PlainText, plaintext};
encrypt(PlainText, {aesgcm, IVLen}) ->
    Key = crypto:strong_rand_bytes(32),
    IVec = crypto:strong_rand_bytes(IVLen),
    {CipherText, CipherTag} = crypto:block_encrypt(aes_gcm, Key, IVec,
                                                   {<<>>, PlainText, 16}),
